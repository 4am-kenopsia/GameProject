...
namespace MapGame
{
	public partial class TestScene : Node
	{
		...
		{
			_turns = 1;
			GD.Print(SaveData.Instance._currentHappiness);
		...
		{
			_mainResourceLabel.Text = $"Main Resource: {SaveData.Instance._currentMagic}";
			_happinessLabel.Text = $"Happiness: {SaveData.Instance._currentHappiness}";
			_thirdResourceLabel.Text = $"Third Resource: {SaveData.Instance._currentTokens}";
			_turnLabel.Text = $"Turns: {_turns}";
		}
		private void BuyBuff()
		{
			// Emit the custom signal
			ResourceManager.Instance.ChangeMagic(300);
			UpdateResourceLabels();
